  ...
  <v-app id="home">
    ...
    <v-container fluid>
      ...
      <v-col cols="12" class="mt-16" id="about">
        <div>
          <v-row>
            ...
            <v-col cols="12" sm="6">
              <h2 class="text mt-n4 text-blue" aria-setsize="32" style="font-family: 'Times New Roman', Times, serif;">About Us</h2>
      ...
      <v-col cols="12" sm="12" id="services">
        <div class="d-flex justify-center mb-6">
          ...
          <v-btn v-model="selectedDepartment" @click="selectDepartment('Internal Medicine')" class="mr-2" variant="tonal"> Medicine</v-btn>
          <v-btn v-model="selectedDepartment" @click="selectDepartment('Dental')" class="mr-2" variant="tonal">Dental</v-btn>
          <v-btn v-model="selectedDepartment" @click="selectDepartment('Gastroenterology')" class="mr-2" variant="tonal">Gastroenterology</v-btn>
      ...
      <div class="d-flex justify-center mb-6">
        <v-container>
            <v-row align="center" justify="center">
              ...
              <v-col cols="auto">
                <v-btn @click="searchHospitals" block rounded="sm" elevation="8" color="blue" size="x-large" class="mr-2">Search</v-btn>
              </v-col>
            </v-row>
        </v-container>
      </div>
    ...
    <v-container>
      <section id="hospitalsList">
        <div v-for="(cardData, index) in displayData" :key="index">
            <v-card
              :loading="loading"
              class="mx-auto my-12"
              max-width="374"
              ...
              <v-card-item>
                <v-card-title > {{cardData.hospital_name}} </v-card-title>
              </v-card-item>
              ...
              <v-card-title>{{cardData.department_name}}</v-card-title>

              <div class="px-4">
                <v-chip-group v-model="selection">
                  <v-chip>Open Hours: 24 * 7 </v-chip>
                </v-chip-group>
              </div>
      ...
      <v-col cols="12" sm="12" class="px-16" id="contact">
        <v-row>
          ...
          <v-col cols="12" sm="8">
            <h1 class="mt-8" style="font-family: 'Times New Roman', Times, serif;">Send your message</h1>
            <v-divider></v-divider>
            ...
            <form @submit.prevent="sendEmail">
              ...
              <v-textarea
                ...
                variant="outlined"
              ></v-textarea>
              <v-btn type="submit" color="yellow" class="mt-2">Submit Now</v-btn>
            </form>
          </v-col>
          </div>
        </v-row>
...
export default defineComponent({
 ...
 data: () => { return {
  //contact form var
    ...
    message: '',
    previousHospital: '',
    searchQuery: 'Gandhinagar', //location query
    selectedDepartment: 'Internal Medicine',
    isDropdownOpen: false,
    showMore: false,
    selectedPrice: 'All',
    selectedRating: 0,
    apiURL:"/hospital/filter",
    ...
    displayData: {}, //empty object
    cardData: "", //empty object
  } },
  computed: {
    // selectedDepartment () {
    //   return '';
  ...
  queryParams() {
     return {
      ...
      minPrice: 0,
      maxPrice:5000, //this.pricemap[this.selectedPrice],//this.pricemap.find(price => price.label === this.selectedPrice).value,//this.pricemap[this.selectedPrice],
...
methods: {
  ...
  try {
    ...
    // for (const card in this.displayData) {
    //   console.log(this.displayData[card].hospital_name);
    // }
    // console.log(this.cardData.hospital_name); //Mahatma hospital
  ...
  // checkCondition(displayData) {
  //   //console.log(typeof displayData)
  //   //console.log(displayData)
  //   //console.log(this.displayData[this.cardData] && this.displayData[this.cardData].hospital_name)
  //   //console.log(this.previousHospital !== this.displayData[this.cardData].hospital_name)

  //   // console.log(this.previousHospital)
  //   // console.log(displayData.hospital_name)
  //   // console.log(this.previousHospital)
  //   if (this.previousHospital !== displayData.hospital_name) {
  //     this.previousHospital = displayData.hospital_name;
  //     console.log(this.previousHospital,displayData.hospital_name,this.previousHospital !== displayData.hospital_name);
  //     return (this.previousHospital !== displayData.hospital_name);
  //   }
  // },
  sendEmail(e) {
    try {
